...
-(UIImage*)depthBufferToImage:(CVPixelBufferRef) pixelBufffer{
    CVPixelBufferLockBaseAddress(pixelBufffer, 0);// 锁定pixel buffer的基地址
    void * baseAddress = CVPixelBufferGetBaseAddress(pixelBufffer);// 得到pixel buffer的基地址
    size_t width = CVPixelBufferGetWidth(pixelBufffer);//360
    size_t height = CVPixelBufferGetHeight(pixelBufffer);//640
//    printf("fuck %d, %d\n",width,height);
//    int testdepth =((__fp16 *)baseAddress)[width + 24] * 10000;
    bool isPlanar = CVPixelBufferIsPlanar(pixelBufffer);
    size_t perRowC = CVPixelBufferGetPlaneCount(pixelBufffer);
    size_t perRowB = CVPixelBufferGetBytesPerRow(pixelBufffer);
//    printf("fuck %d %d %d\n",(int)isPlanar, perRowB, perRowC);
    byte *array = (byte *) malloc(width*height*4);
    
    for (int j = 0; j<height; j++) {
        for (int i =0; i<width; i++) {
            int index = j * width + i;
            int delta = (32 - width % 32) % 32;
            int strIndex = j * (width + delta) + i;
            //ARGB
//        lxk: change one channel depth to channel 4
            int depth =((__fp16 *)baseAddress)[strIndex] * 10000;
