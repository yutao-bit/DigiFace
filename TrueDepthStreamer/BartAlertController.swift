...
//  TrueDepthStreamer
//
//  Created by iosDeveloper on 2022/7/26.
...
//    private var buttonHeight:CGFloat!
    ...
    private func creatActionButton(){
        ...
        for (index, obj) in self.actions.enumerated(){
            ...
            if obj.ActionBGColor != UIColor.white{
                ...
                btn.setBackgroundImage(UIColor().creatImageWithColor(color: UIColor.black.withAlphaComponent(0.1)), for: UIControl.State.highlighted)
                
                
            }else{
                
                btn.setBackgroundImage(UIColor().creatImageWithColor(color: UIColor.init(white: 0.97, alpha: 1)), for: UIControl.State.highlighted)
    ...
    private func updateMessageLabelFrame(){
        ...
        let messageY = self.AlertTitle.count > 0 ? self.titleLabel.frame.maxY + 20 : contentMargin.top
        
        if self.AlertMessage.count > 0 {
    ...
    private func updateAllSeparatorLine(){
        ...
        if self.actions.count > 0{
            ...
            let offsetAmount = (self.AlertTitle.count > 0 || self.AlertMessage.count > 0) ? 0 : 1
    ...
    private func getLineCount() -> Int {
        ...
        //如果没有标题和内容，分割线条数就减一
        linesAmount -= (self.AlertTitle.count > 0 || self.AlertMessage.count > 0) ? 0 : 1
